...
XY <- cbind(X.scale, Y)
colnames(XY)[colnames(XY) == "Y"] <- "target_obl"
...
colnames(XY)[colnames(XY) == "target_obl"] <- "target"
...
idx.tdur <- which(colnames(X) == "tdur")
penalty.factor[idx.tdur] <- 0
...
write.csv(obl.result.or, 
          file=here::here('data/output/result_obl.csv'),
          row.names=FALSE)
```
