...
index.sparse <- sparsity(X, fraction=0.75)
X <- X[,-index.sparse]
X <- data.frame(X)
...
nonzero.coef.count <- sum(coef(fit.lasso, s = "lambda.min")[-1] != 0)
index.nonzero <- which(coef(fit.lasso, s = "lambda.min")[-1] != 0)
# preval preds
index.optim <- which(fit.lasso$lambda == fit.lasso$lambda.min) 
...
coefficient_values <- coef(fit.lasso, s = "lambda.min")
...
coefs <- coef(fit.lasso, s = "lambda.min")
...
lambda_min <- fit.lasso$lambda.min / nrow(X)
...
write.csv(obl.result.or, 
          file=here::here('data/output/result_obl_pgs12.csv'),
          row.names=FALSE)
```
